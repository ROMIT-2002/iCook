...
// Local-dev twin of api/feedback.ts. The feedback email is sent from the
// browser because the relay rejects server-side requests; this route only
// mirrors the Vercel log record so submissions survive a relay outage.
app.post('/api/feedback', async (req, res) => {
  try {
    ...
    const guest =
      attribution === 'anonymous' ? 'Anonymous Guest' : (name || '').trim() || 'Unnamed Guest';
    ...
    console.log(`Guest: ${guest} (${attribution})
Decorations: ${decorations}/5
Event Planning: ${eventPlanning}/5
Food Quality: ${foodQuality}/5
Food Diversity: ${foodDiversity}/5
Overall average: ${average}/5
Comments: ${comments && comments.trim() ? comments.trim() : 'None'}
